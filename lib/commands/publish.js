...
function scanSiteMatdataTable(config) {
  return new Promise((resolve, reject) => {
...
function batchUpdateSiteMetadataTable(config, batch) {
  return new Promise((resolve, reject) => {
...
function executeUpdateSiteMetadataTableBatches(config, actions) {
  ...
  for (const batch of actionBatches) {
    chain = chain.then(
      curUA => batchUpdateSiteMetadataTable(config, batch).then(
...
Proceed? (yes/no) `, answer => {
  ...
  .then(([oldSiteMetadata, newSiteMetadata]) => {
    ...
    if (actions.length === 0) {
      ...
      return executeUpdateSiteMetadataTableBatches(config, actions)
      .then(unprocessedActions => {
        if (unprocessedActions.length > 0) {
          console.log("not all updates were successful" +
            ", will retry in 5 seconds...");
          return new Promise(resolve => {
            setTimeout(() => resolve(
              executeUpdateSiteMetadataTableBatches(config, unprocessedActions)
            ), 5000);
          });
        }
      })
      .then(unprocessedActions => {
        if (unprocessedActions && unprocessedActions.length > 0) {
          return Promise.reject(new Error(
            "Unable to update site metadata table. Unprocessed actions are:\n" +
            JSON.stringify(unprocessedActions, null, "  ")
          ));
        }
      });
    }
