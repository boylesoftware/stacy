...
const stacy = require("../runtime.js");
...
exports.execute = function(projectDir, config, env, cmd) {
  ...
  return new Promise((resolve, reject) => {
    ...
    watcher.on("all", (event, path) => {
      if (common.TEMPLATE_FILE_PATTERN.test(path)) {
        const m = path.match(/([^/]*?)(?:\.([^.]+))?\.[^.]+$/);
        const entryType = m[1];
        const ext = m[2] || "html";
        ...
        const relPath = path.substring(templatesDir.length);
        switch (event) {
        case "add":
        case "change":
          console.log(`compiling template at ${relPath}...`);
