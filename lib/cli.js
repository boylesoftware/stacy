...
.option(
  "--cf-access-key <key>",
  "Contentful Content Delivery API access key"
)
.option(
  "--cf-host <host>",
  "Contentful Content Delivery API host",
  "cdn.contentful.com"
)
.action((siteId, cmd) => {
  cmd.siteId = siteId;
  if (common.isEmpty(cmd.cfSpace)) {
    console.error("error: missing option '--cf-space'");
  }
  else if (common.isEmpty(cmd.cfAccessKey)) {
    console.error("error: missing option '--cf-access-key'");
  }
  else {
    executeCommand("new", cmd);
  }
});
